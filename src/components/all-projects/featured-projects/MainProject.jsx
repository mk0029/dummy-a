    ...
    <>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 w-full mt-8">
        {FEATURED_PROJECT_DATA_LIST.map((obj, i) => (
          <div
            key={i}
            onClick={() => router.push(obj.title.toLowerCase().replace(/ /g, '-').replace('|', '').replace('--', '-'))}
            className={`${i >= 4 && !showAll ? "opacity-0 h-0 overflow-hidden" : "opacity-100 h-auto"} transition-all duration-500`}
          >
