          ...
          <div className="flex gap-4">
            <div className="flex flex-col gap-2.5 w-1/2 lg:max-w-[250px]">
              {FOOTER_LINK_LIST.map((obj, i) => {
                return (
                  <Link
                    className="text-xs text-light-gray md:text-sm lg:text-base hover:text-dark-orange !leading-130 relative before:absolute hover:before:w-full duration-300 before:duration-300 transition-all max-w-max before:w-0 before:h-0.5 before:bg-light-gray/50 before:bottom-0 before:left-0"
                    key={i}
                    href={obj.url}>
                    {obj.title}
                  </Link>
